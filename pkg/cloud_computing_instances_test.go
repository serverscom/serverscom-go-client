...
func TestCloudComputingInstancesCreate(t *testing.T) {
	...
	g.Expect(cloudInstance.RegionID).To(Equal(int64(1)))
	g.Expect(cloudInstance.RegionCode).To(Equal("test"))
...
func TestCloudComputingInstancesReboot(t *testing.T) {
	g := NewGomegaWithT(t)

	ts, client := newFakeServer().
		WithRequestPath("/cloud_computing/instances/BDbDxbl2/reboot").
		WithRequestMethod("POST").
		WithResponseBodyStubFile("fixtures/cloud_instances/reboot_response.json").
		WithResponseCode(202).
		Build()
	...
	cloudInstance, err := client.CloudComputingInstances.Reboot(ctx, "BDbDxbl2")

	g.Expect(err).To(BeNil())
	g.Expect(cloudInstance).ToNot(BeNil())

	g.Expect(cloudInstance.ID).To(Equal("BDbDxbl2"))
	g.Expect(cloudInstance.Status).To(Equal("REBOOTING"))
	g.Expect(cloudInstance.Name).To(Equal("some"))
	g.Expect(cloudInstance.RegionID).To(Equal(int64(1)))
	g.Expect(cloudInstance.RegionCode).To(Equal("test"))
	g.Expect(cloudInstance.FlavorID).To(Equal("101"))
