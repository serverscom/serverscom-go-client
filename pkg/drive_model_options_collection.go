...
const (
	driveModelOptionListPath = "/locations/%d/order_options/server_models/%d/drive_models"
)

// DriveModelOptionsCollection is an interface for interfacing with the collection of DriveModelOption
...
type DriveModelOptionsCollection interface {
	...
	NextPage(ctx context.Context) ([]DriveModelOption, error)
	PreviousPage(ctx context.Context) ([]DriveModelOption, error)
	FirstPage(ctx context.Context) ([]DriveModelOption, error)
	LastPage(ctx context.Context) ([]DriveModelOption, error)

	Collect(ctx context.Context) ([]DriveModelOption, error)
	List(ctx context.Context) ([]DriveModelOption, error)
...
func NewDriveModelOptionsCollection(client *Client, LocationID int64, ServerModelID int64) DriveModelOptionsCollection {
	return &DriveModelOptionsCollectionHandler{
		...
		clean:      true,
		collection: make([]DriveModelOption, 0),
	}
}
...
// NextPage navigates to the next page returns a []DriveModelOption, produces an error, when a
...
func (col *DriveModelOptionsCollectionHandler) NextPage(ctx context.Context) ([]DriveModelOption, error) {
	return col.navigate(ctx, "next")
}

// PreviousPage navigates to the previous page returns a []DriveModelOption, produces an error, when a
...
// You can force to load pagination metadata by calling Refresh or List methods.
func (col *DriveModelOptionsCollectionHandler) PreviousPage(ctx context.Context) ([]DriveModelOption, error) {
	return col.navigate(ctx, "prev")
}

// FirstPage navigates to the first page returns a []DriveModelOption, produces an error, when a
// collection has no first page.
//
// Before using this method please ensure IsClean returns false and HasFirstPage returns true.
// You can force to load pagination metadata by calling Refresh or List methods.
func (col *DriveModelOptionsCollectionHandler) FirstPage(ctx context.Context) ([]DriveModelOption, error) {
	return col.navigate(ctx, "first")
}

// LastPage navigates to the last page returns a []DriveModelOption, produces an error, when a
// collection has no last page.
//
// Before using this method please ensure IsClean returns false and HasLastPage returns true.
// You can force to load pagination metadata by calling Refresh or List methods.
func (col *DriveModelOptionsCollectionHandler) LastPage(ctx context.Context) ([]DriveModelOption, error) {
...
func (col *DriveModelOptionsCollectionHandler) Collect(ctx context.Context) ([]DriveModelOption, error) {
	var accumulatedCollectionElements []DriveModelOption
...
func (col *DriveModelOptionsCollectionHandler) List(ctx context.Context) ([]DriveModelOption, error) {
	if col.IsClean() {
...
func (col *DriveModelOptionsCollectionHandler) fireHTTPRequest(ctx context.Context) error {
	...
	initialURL := col.client.buildURL(driveModelOptionListPath, col.LocationID, col.ServerModelID)
...
func (col *DriveModelOptionsCollectionHandler) navigate(ctx context.Context, name string) ([]DriveModelOption, error) {
	if col.IsClean() {
		if err := col.Refresh(ctx); err != nil {
			return nil, err
		}
